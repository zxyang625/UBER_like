...
var serviceName = fs.String("service-name", "trip", "default service name")
...
func Run() {
	...
	logger = config.GetKitLogger(config.SystemTrip)
...
func getEndpointMiddleware(logger kitlog.Logger) (mw map[string][]endpoint1.Middleware) {
	mw = map[string][]endpoint1.Middleware{
		"GenTrip": {
			...
			endpoint.InstrumentingMiddleware(promtheus.NewHistogram(config.SystemTrip, config.MethodGenTrip, "GenTrip histogram")),
			endpoint.CountingMiddleware(promtheus.NewCounter(config.SystemTrip, config.MethodGenTrip, "GenTrip count")),
			endpoint.TraceEndpoint(tracer.NativeTracer, config.MethodGenTrip+"/service"),
		},
	}
...
func initGRPCGateway(g *group.Group) {
	...
	// HTTP转grpc
	err := pb.RegisterTripHandlerFromEndpoint(context.Background(), mux, *grpcAddr, opts)
	if err != nil {
