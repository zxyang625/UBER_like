	...
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, request interface{}) (response interface{}, err error) {
			defer func(begin time.Time) {
				duration.With(config.SystemTrip+"_histogram", fmt.Sprint(err == nil)).Observe(time.Since(begin).Seconds())
			}(time.Now())
...
func CountingMiddleware(count metrics.Counter) endpoint.Middleware {
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, request interface{}) (response interface{}, err error) {
			defer func(begin time.Time) {
				count.With(config.SystemTrip+"_counter", fmt.Sprint(err == nil)).Add(1)
			}(time.Now())
			return next(ctx, request)
		}
	}
