...
func New(instance string, options map[string][]http.ClientOption) (service.TripService, error) {
	...
	{
		genTripEndpoint = http.NewClient("POST", copyURL(u, "/gen-trip"), encodeHTTPGenericRequest, decodeGenTripResponse, options["GenTrip"]...).Endpoint()
	}
...
// decodeGenTripResponse is a transport/http.DecodeResponseFunc that decodes
...
func decodeGenTripResponse(_ context.Context, r *http1.Response) (interface{}, error) {
	if r.StatusCode != http1.StatusOK {
		return nil, http2.ErrorDecoder(r)
	}
	var resp endpoint1.GenTripResponse
