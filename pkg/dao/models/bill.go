...
func GetBill(billNum int64) (*Bill, error) {
	bill := &Bill{}
	err := db.Model(&Bill{}).Select([]string{"bill_num", "price", "start_time", "end_time", "origin", "destination", "passenger_name", "driver_name", "payed", "user_id"}).
...
func GetBillList(userId int64) ([]*Bill, error) {
	...
	err := db.Model(&Bill{}).Where("user_id = ?", userId).Find(&billList).Error
	if err != nil {
		return nil, err
	}
	return billList, nil
